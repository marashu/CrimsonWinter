...
$app_id = APP_ID;
    ...
    $my_url = "http://apps.facebook.com/crimsonwinter/bank.php";
    
//initialize achievement variables
   $displayAchievement = 0;
$achName = '';

//login details same as index.php
...
$query = "SELECT achievement19 FROM CW_Achievements WHERE id='" . $uid . "'";
$result = mysql_query($query);
$row = mysql_fetch_row($result);
$KrevelHorde = $row[0];
...
//withdrawing and depositing krevels from bank to protect from pvp
$error = 0;
$errorMsg = '';
$amount = 0;
if(isset($_POST[check]))
{
	if($_POST[check] == "withdraw")
	{
		...
		{
			$error = 1;
			$errorMsg = "You must withdraw at least 1 krevel.";
		}
		else if($_POST[amount] > $bank)
		{
			$error = 1;
			$errorMsg = "Not enough krevels in the bank.";
		}
		else
		{
			...
			$bank -= $amount;
			$krevels += $amount;
			$query = "UPDATE CW_Users SET krevels='" . $krevels . "', bank='" . $bank . "' WHERE id='" . $uid . "'";
			$result = mysql_query($query);
		}
	}
	else if($_POST[check] == "deposit")
	{
		if(!is_numeric($_POST[amount]))
		{
			$error = 1;
		...
		{
			$error = 1;
			$errorMsg = "You must deposit at least 1 krevel.";
		}
		else if($_POST[amount] > $krevels)
		{
			$error = 1;
			$errorMsg = "Not enough krevels.";
		}
		else
		{
			$amount = (int)($_POST[amount]);
			...
			if($amount >= 10)
			{
				$amount = $amount - round($amount * 0.1);
			}
			else
			{
				$amount--;
			}
			if($KrevelHorde >= 0 && $amount >= 500000)
			{
				$KrevelHorde = -1;
				$skills++;
				$query = "UPDATE CW_Users SET skills=skills+1 WHERE id='" . $uid . "'";
				$result = mysql_query($query);
				$query = "UPDATE CW_Achievements SET achievement19='" . $KrevelHorde . "' WHERE id='" . $uid . "'";
				$result = mysql_query($query);
$displayAchievement = 19;
$achName = "Krevel Horde";
			}
			$bank += $amount;
			$query = "UPDATE CW_Users SET krevels='" . $krevels . "', bank='" . $bank . "' WHERE id='" . $uid . "'";
			$result = mysql_query($query);
	...
	{
		$error = 1;
		$errorMsg = "Something went wrong with posting your request.  Please try again.";
	}
...
<div style="position:absolute; top:133px; left:8px; width:442px; height:322px; background-image:url('assets/Box_Bank.png'); color:black;">
<div style="position:absolute; top:8px; left:28px; width:360px; text-align:center; color:orange;">BANK</div><br/><br/>
<div style="position:absolute; top:48px; left:28px; width:360px; text-align:center;">
Keep your krevels safe from greedy Chosen who would steal them from you.<br/><br/>

"I'll store your krevels for you," Ticket offers,"but I keep ten percent!"<br/><br/>(Balance: $bank krevels)
</div>



<div style='position:absolute; top:190px; left:60px; text-align:center;'>
<form method="post" action="bank.php"><input type="text" name="amount" size="12" maxlength="12" value="0" /><br/><br/>
<input type="hidden" name="check" value="withdraw"/><input type="hidden" name="token" value="$access_token"/><input type="image" value="Withdraw" src="assets/BTN_Withdraw.png"/></form>
</div>

<div style='position:absolute; top:190px; left:240px; text-align:center;'>
<form method="post" action="bank.php">
_END;
echo "<input type='text' name='amount' size='12' maxlength='12' value='" . $krevels . "' /><br/><br/>";
echo <<<_END
<input type="hidden" name="check" value="deposit"/><input type="hidden" name="token" value="$access_token"/><input type="image" value="Deposit" src='assets/BTN_Deposit.png'/></form>
</div>
...
if($amount > 0)
{
	if($_POST[check] == "deposit")
		echo "Deposited " . $amount . " krevels.";
	if($_POST[check] == "withdraw")
		echo "Withdrew " . $amount . " krevels.";
}
echo "</div>";
}
echo <<<_END
...
<div style="position:absolute; top:140px; left:368px; width:368px; height:296px; background-image:url('assets/ImageTicket.png');">
</div>

<div style='position:absolute; top:450px; left:520px; font-size:0.75em;'>
